...
import { formatTimestamp, parseText, sleep, convertUTC } from "../utils.js";
...
class StarRailExtractor extends BaseExtractor {
  ...
  async *fetchData(url = null) {
    ...
    for (const gachaType of GACHA_TYPES) {
      ...
      while (true) {
        const { timezone, list } = yield* this.#fetchPage(
          gachaTypeName,
          finalUrl,
          page,
          endId,
        ...
        items.push(
          ...list.map((item) => {
            return {
              ...
              timestamp: convertUTC(new Date(item.time).getTime(), timezone),
  ...
  importNative(payload) {
    if (Array.isArray(payload) && payload.length > 0) {
      const grouped = payload.reduce((acc, item) => {
        const key = `${item.uid}_${item.gachaType}`;

        acc[key] ??= {
          groupKey: { uid: item.uid, gachaType: item.gachaType },
          items: [],
        };
      ...
      for (const { groupKey, items } of Object.values(grouped)) {
        ...
        this.#save(groupKey, sortedItems);
      }
    }
  }
  ...
  async importUIGF41(payload) {
    ...
    for (const data of payload) {
      ...
      const groupedItems = data.list.reduce((acc, item) => {
        ...
        acc[gachaType].push({
          ...
          itemTypeNorm: parseItemType(item.item_type),
          rankType: item.rank_type ?? "???",
          timestamp: convertUTC(time, timezone),
          fetchTimestamp: fetchTimestamp,
        });
        return acc;
      }, {});

      for (const [gachaType, items] of Object.entries(groupedItems)) {
        this.#save(
          { uid, gachaType },
          items.sort((a, b) => {
            const idA = BigInt(a.id);
